...
function fetchMFIEpic(action$) { // action$ is a stream of actions
    // action$.ofType is the outer Observable
    return action$
        .ofType(FETCH_MFI) // ofType(FETCH_WHISKIES) is just a simpler version of .filter(x => x.type === FETCH_WHISKIES)
        .switchMap(() => {
        ...
        .map(whiskies => fetchMFISuccess(whiskies)) // map the resulting array to an action of type FETCH_WHISKIES_SUCCESS
...
export const rootEpic = combineEpics(fetchMFIEpic);